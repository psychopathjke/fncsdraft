...
'      // Read the terms through the same function the window and the contract',
'      // read them through, rather than off the row. Nobody has a written-in',
'      // cut, so sorting on a.cut sorted on nothing at all and every agent',
'      // looked identical: it held only while the terms happened to be the',
'      // ordinary ones for everybody, and stopped holding the moment they ran',
'      // down his list on 20 August. See ccAgentTermsOf.',
'      var cutOf = function(a){ return ccAgentTermsOf(a).cut; };',
'      var dear = CC_AGENTS.slice().sort(function(a,b){ return cutOf(b) - cutOf(a); })[0];',
'      var cheap = CC_AGENTS.slice().sort(function(a,b){ return cutOf(a) - cutOf(b); })[0];',
'      check("they are not the same man", dear.name !== cheap.name,',
'            dear.name + " / " + cheap.name);',
'      fresh(90);',
...
'      check("on his own share", Math.abs(terms.cut - cutOf(cheap)) < 1e-9,',
'            String(terms.cut));',
...
'            Math.abs(careerAgentTerms().cut - cutOf(dear)) < 1e-9);',
'',
