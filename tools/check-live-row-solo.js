// То же самое, но на Соло Серии: там лобби на сто человек и одиночный состав,
// и его скрин («во время игры не насчитывается») — именно оттуда.
//
...
//
//   node tools/check-live-row-solo.js
...
(async function(){
  ...
  try{
    ...
    for(let d=CC_YEAR_FROM; d<=CC_YEAR_TO; d=ccAddDays(d,1))
      if((days.get(d)||[]).some(e=>e.kind==='solo')){ day=d; break; }
    ...
    const rows=[...seen.entries()].map(([n,s])=>({game:n, shots:s.size,
      pows:new Set([...s].map(x=>x.split('|')[3])).size, what:[...s].slice(0,14)}));
