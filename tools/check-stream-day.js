...
(async function(){
  ...
  try{
    ...
    /* Подписи автора в кадре быть НЕ должно: его слово 11 сентября — «убрать footage
       чей». Ролики он выбрал сам, и строка в углу читалась как чужая метка. */
    if(slot.querySelector('.tv-clip-by')) fail('в кадре осталась подпись автора');
    ...
    /* Рисунок под роликом есть всегда, но ПОКАЗЫВАЕТСЯ не сразу: первые секунды
       эфира кадр чёрный, как на настоящем стриме, а рисунок включается, только если
       плеер так и не заиграл (его слово 11 сентября про «нарисованную фигню»). */
    { const fb2=document.querySelector('.tv-player.live .tv-fb');
      if(!fb2) fail('под роликом нет рисунка');
      if(fb2 && fb2.dataset.on) fail('рисунок показан сразу, не дождавшись плеера');
      if(fb2 && +getComputedStyle(fb2).opacity!==0) fail('рисунок видно до того, как плеер сдался'); }
    out.steps.push('ролик не идёт — рамка прозрачна, кадр чёрный');
    ...
    { ccTvClipPlaying();
      ...
      const fb3=document.querySelector('.tv-player.live .tv-fb');
      if(fb3 && fb3.dataset.on) fail('рисунок остался поверх заигравшего ролика');
      out.steps.push('плеер сказал «играю» — ролик виден, рисунка нет'); }
    /* Кусок доиграл — кадр гаснет на время перезапуска: иначе поверх стоп-кадра
       выкладывается шапка YouTube, а перезагрузка рамки начинается с логотипа. */
    { if(typeof ccTvClipLoop==='function'){ ccTvClipLoop();
        if(slot.dataset.on) fail('кадр не погас на перезапуске — будет видна шапка YouTube');
        out.steps.push('перезапуск куска идёт при чёрном кадре'); } }
