// ПАРА В ГОНКЕ: два живых клиента через настоящий воркер, у каждого своя карьера, но вечер они
// играют ОДНОЙ командой (careerRacePairAsk/Yes): состав — две карточки людей, сила и адрес
// вопросов одни у обоих, таблицы обязаны совпасть, в поле — одна команда людей, у каждого своя
// строка журнала с одним и тем же местом. Собран из check-race-live-two.js (22.09).
//
//   node tools/check-race-pair.js            (кубок Д1, 2026-02-02)
//   CC_HOST=ws://127.0.0.1:8787 node tools/check-race-pair.js   (локальный воркер)
...
(async function(){
  ...
  document.getElementById('__out').textContent='PB'+'EGIN'+encodeURIComponent(JSON.stringify(out))+'PE'+'ND';
    ...
    // ПАРА: A зовёт B, B принимает; оба ждут, пока пара встанет у обоих (строки гонки с pair).
    if(${JSON.stringify(who.role_mp)}==='a'){ for(let i=0;i<50 && !ccRaceRival();i++) await wait(100); careerRacePairAsk(ccRaceRival().id); }
    for(let i=0;i<600 && !ccRacePairOn();i++){ if(CC_RACE_PAIR_ASK) careerRacePairYes(); await wait(100); }
    out.notes.pair={on:ccRacePairOn(), id:ccRacePairId(), cards:(ccRacePairCards()||[]).map(c=>c.handle), line:ccRaceMyLine().mates.map(m=>m.handle), bench:(ccRaceMyLine().bench||[]).map(m=>m.handle)};
    if(!ccRacePairOn()) throw new Error('пара не встала: '+JSON.stringify(out.notes.pair)+' ask '+JSON.stringify(CC_RACE_PAIR_ASK)+' race '+JSON.stringify(CAREER.career.race)+' peers '+JSON.stringify(Object.keys(CC_RACE_PEERS).map(k=>k+':'+CC_RACE_PEERS[k].pair)));
...
(async ()=>{
  ...
  for(const [n, r] of [['A', a], ['B', b]]){ const t=(r.notes.table||[]).join('\n');
    ...
    if(!FF && (!r.notes.f1 || r.notes.f1.you<0 || r.notes.f1.rival>=0)){ console.log('FAIL '+n+': в поле должна быть ОДНА команда людей — пара: '+JSON.stringify(r.notes.f1)); bad++; }
    if(!r.notes.pair || !r.notes.pair.on || (r.notes.pair.cards||[]).length!==2){ console.log('FAIL '+n+': пара не встала: '+JSON.stringify(r.notes.pair)); bad++; }
    if(r.notes.f1 && !/livea|liveb/.test(String(r.notes.f1.youKey))){ console.log('FAIL '+n+': состав вечера не из двух людей: '+r.notes.f1.youKey); bad++; }
    console.log('   '+n+': пара '+JSON.stringify(r.notes.pair));
  ...
  if(a.notes.mine && b.notes.mine && a.notes.mine.place!==b.notes.mine.place){ console.log('FAIL у пары разные места: '+a.notes.mine.place+' / '+b.notes.mine.place); bad++; }
  if(bad) process.exit(1);
  console.log('пара в гонке: два живых клиента сыграли один вечер одной командой');
