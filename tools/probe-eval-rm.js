...
(async function(){
  ...
  try{
    ...
    for(let d=CC_YEAR_FROM; d<=CC_YEAR_TO; d=ccAddDays(d,1))
      if((days.get(d)||[]).some(e=>e.kind==='eval')){ day=d; break; }
...
const dom = execFileSync(CHROME, ['--headless=new','--disable-gpu','--no-sandbox','--force-prefers-reduced-motion',
