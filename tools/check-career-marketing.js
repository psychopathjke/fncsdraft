...
'',
'      // ---- the list runs downhill ---------------------------------------',
'      // It ran off the follower count until 20 August and runs off his own',
'      // order now, so this reads the real rows rather than two made-up counts:',
'      // a row that is not in the list has no place in it. See ccRankPull.',
'      var mrows = ccByHand(CC_MARKETING);',
'      var small = ccMktTermsOf(mrows[mrows.length-1]);',
'      var big = ccMktTermsOf(mrows[0]);',
'      out.notes.curve = {top: big, bottom: small};',
'      check("the top of the list asks more", big.cost > small.cost, small.cost + " vs " + big.cost);',
'      check("and lands more", big.rate > small.rate, small.rate + " vs " + big.rate);',
'      var mcost = mrows.map(function(m){ return ccMktTermsOf(m).cost; });',
'      check("and every step down is a step down",',
'            mcost.filter(function(c,i){ return i && mcost[i-1] < c; }).length === 0,',
'            mcost.join(" "));',
'',
