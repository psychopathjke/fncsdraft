...
'      // Снимок европейской таблицы ДО всех переездов — с ним сверяемся в конце.',
'      var dePing0=ccPingOf("de","EU");',
'      out.notes.per={};',
...
'        // ">0", а не "===0": ccBuildMap возвращает <div class="cc-mapzoom">,',
'        // то есть имя класса стоит на 12-м знаке, а не на нулевом. Требование',
'        // нуля не выполнялось НИ У ОДНОГО региона, включая Европу, — сторож',
'        // был красным всегда и мерил не карту, а свою опечатку.',
...
'      // И СРАВНЕНИЕ, а не круглое число. Порог «больше 80» был выдуман: он',
'      // ничего не говорит про то, зачем сдвиг нужен. Мера смысла тут одна —',
'      // сколько человек попало бы в Дивизион 1 по ЕВРОПЕЙСКОЙ лестнице.',
'      // Замер: по европейской — ОДИН человек на 640, со сдвигом — 65.',
'      var d1Flat=careerRosterNowEU().filter(function(p){',
'        return (p._ovr!=null?p._ovr:attrsFor(p).ovr)>=CC_DIV_RATING[1]; }).length;',
'      out.notes.oceDivisionOne={shifted:d1, european:d1Flat};',
'      check("and Oceania has a Division 1 to play in — the shift is what makes it",',
'            d1>40 && d1>10*Math.max(1, d1Flat),',
'            d1+" on its own ladder against "+d1Flat+" on the European one");',
'',
...
'      /* Германия сверяется САМА С СОБОЙ, а не с числом 1. Смысл проверки —',
'         «переезд в Океанию не портит европейскую таблицу», и он держится на',
'         сравнении «до» и «после». Литеральная единица была снята с таблицы',
'         пингов того дня; таблицу с тех пор пересчитали (Германия — 7 мс), и',
'         сторож краснел на верных данных. */',
'      check("and Germany kept the millisecond it had before the trip",',
'            ccPingOf("de","EU")===dePing0, ccPingOf("de","EU")+" was "+dePing0);',
'',
'      done();',
