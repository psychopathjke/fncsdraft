  ...
  try{
    localStorage.setItem('fncsdraft_career', JSON.stringify({
      ...
      career:{season:1, day:'2026-02-02', division:1, earnings:0, balance:1000, reach:0,
              tokens:[], log:[], news:[],
              // Февраль дуо-года — остров m1 (careerBrSet по сезону, 16.09).
              spots:{m1:[{i:0, aura:6, won:6, day:'2026-01-05'}],
    ...
    careerSpotOpenFor('m1');
    careerSpotChoose(11);
    if(document.getElementById('ccAskModal').style.display==='flex') ccAskGo(true);
    if(CC_SPOT_OPEN) fail('вторая карта осталась открытой');
    if(careerSpotList('m1')[0].i!==11) fail('вторая лока не сменилась');
