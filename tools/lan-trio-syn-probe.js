...
(async function(){
  ...
  try{
    localStorage.setItem('fncsdraft_career', JSON.stringify({
      ...
      career:{season:2, size:3, day:'2026-05-29', division:1, earnings:0, balance:0, reach:0,
              tokens:[], log:[], news:[]},
      partner:null}));
    careerEntry(); CARD_MODE=true; squadSize=3;
    const me=careerCard();
    const field=careerSummitField('final', {isYou:true, squad:[me,me,me]}, [me,me,me]);
    const byReg={};
    field.filter(t=>!t.isYou).forEach(t=>{
      ...
      r.n++; r.pow+=t.pow||0; r.ovr=(r.ovr||0)+((t.squad||[]).reduce((q,c)=>q+((attrsFor(c)||{}).ovr||0),0)/((t.squad||[]).length||1));
    ...
    Object.keys(byReg).forEach(k=>{
      ...
      out.regions[k]={n:r.n, avgPow:Math.round(r.pow/r.n*10)/10,
        avgOvr:Math.round(r.ovr/r.n*10)/10, avgSyn:Math.round(r.syn/r.n*10)/10, avgDuo:Math.round(r.duo/r.n*10)/10,
