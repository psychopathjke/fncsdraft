...
'    // Докрутить каждую ленивую плитку до экрана и дёрнуть событие, которым',
'    // страница красит их сама.',
'    //',
'    // Раньше здесь был один прыжок вниз и четыреста миллисекунд ожидания, и он',
'    // работал, пока вся стена плиток помещалась в экран плюс те 600px, что',
'    // просит наблюдатель: арт красился вообще без прокрутки, а прыжок был',
'    // формальностью. С плиткой «весь год» страница стала выше, нижний ряд ушёл',
'    // за поле наблюдателя — и проба превратилась в гонку с кадром, который в',
'    // headless приходит когда захочет: падала примерно каждый третий раз, ничего',
'    // не сломав. Прокрутка шагами гонку не убрала.',
'    //',
'    // IntersectionObserver ждать надёжно нельзя, а вот запасной путь страницы',
'    // (cc-screen-shown) синхронный и красит теми же руками. Так проверка',
'    // осталась про краску, а не про разметку, и перестала врать через раз.',
'    [].slice.call(document.querySelectorAll("[data-art]")).forEach(function(el){',
'      el.scrollIntoView({block: "center"});',
'      window.dispatchEvent(new Event("cc-screen-shown"));',
'    });',
'    window.scrollTo(0, 0);',
'    setTimeout(function(){',
'      try{',
...
'        /* И арт карьеры. С 4 сентября карточки карьеры в рядах нет —',
'           его слово «убрать вообще панельку fncs career… внизу типо», — а',
'           карьера живёт своей полосой под героем (.cstrip). Смотрим на неё:',
'           плитка была тем же самым, только в ряду. */',
'        /* 7 сентября полоса влилась в героя — арт карьеры теперь арт героя. */',
'        var car = document.querySelector(".cstrip-art") || document.querySelector(".hero-art") || document.getElementById("careerArt");',
'        var cbg = car ? getComputedStyle(car).backgroundImage : "";',
'        out.notes.career = cbg.slice(0, 90);',
'        if (!car) out.fails.push("полоса карьеры пропала с главной");',
'        else if (cbg.indexOf("url(") < 0) out.fails.push("полоса карьеры без арта — " + cbg);',
'        done();',
