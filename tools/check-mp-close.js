  ...
  try {
    ...
    ['runCareerCup','runCareerMajor','runCareerSummit','runCareerGlobals','runCareerGclc',
     ...
     'runCareerVictory','runCareerSoloSeries'].forEach(fn => {
      ...
      /* И КАЖДЫЙ шаг дня в раннере стоит после закрытия. Его отчёт, 29 августа:
         «две сессии сыграл — после него опять дивижен»: вторая сессия кубка и
         второй путь Reload двигали день без close, а в команде такой шаг глух. */
      let at2=0, loose=0;
      while((at2=body.indexOf('careerAdvanceTo(', at2))>=0){
        if(body.slice(Math.max(0, at2-900), at2).indexOf('ccMpClose(')<0) loose++;
        at2+=10;
      }
      check(fn + ': каждый шаг дня — после закрытия', loose===0, loose+' без close');
    });
