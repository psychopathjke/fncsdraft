...
(function(){
  ...
  try{
    ...
    // Прибавка применяется ТАК ЖЕ, как в игре: после старта, на восьмой зоне,
    // через тот же пошаговый движок. Первая версия пробы добавляла силу до
    // simulateGameOnMap — а он сам переписывает _pf/_pc в начале, и замер
    // показывал ровный ноль на любой прибавке.
      ...
      for(let g=0; g<GAMES_N; g++){
        ...
        const game=ZoneSim.simulateZoneGame(field, {
          ...
          startOf:t=>{ const z=t.landingZone;
            return z?{x:z.x+z.w/2, y:z.y+z.h/2}:{x:50,y:50}; },
          duel:(a,b,dropping)=>dropping?resolveDropDuel(a,b):resolveDuel(a,b,exponent)});
        game.playTo(8);
        const got = coin ? (Math.random()<CC_HG_ODDS ? add : -CC_HG_FAIL) : add;
        if(got) ccAddGamePow(you, got);
    ...
    out.rows.push({what:'ничего (лоуграунд)', add:0, ...run(0)});
    out.rows.push({what:'рефреш', add:CC_REFRESH_POW, ...run(CC_REFRESH_POW)});
    out.rows.push({what:'хайграунд взят', add:CC_HG_POW, ...run(CC_HG_POW)});
    // Как это выглядит из кресла игрока: монетка на каждой игре.
