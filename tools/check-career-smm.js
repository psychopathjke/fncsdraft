...
'      // His call, 29 August: the SMM tile chooses like the coach — hiring another replaces the first.',
'      check("hiring the same one again is refused", careerHireSmm(DEAR.id) === false);',
'      check("hiring another replaces the first", careerHireSmm(CHEAP.id) === true && ccSmm() && ccSmm().id === CHEAP.id);',
'      careerEndSmm(); careerHireSmm(DEAR.id);',
