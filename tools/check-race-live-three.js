// Гонка ВТРОЁМ в одной комнате: три живых клиента через воркер (CC_HOST для
// локального wrangler dev), три карьеры, три разных бот-напарника — три таблицы
// обязаны совпасть до хеша. Собран из check-race-live-two.js (scratchpad
// make-race-three.js).
//
//   CC_HOST=ws://127.0.0.1:8787 node tools/check-race-live-three.js
...
(async function(){
  ...
  try{
    ...
    for(let i=0;i<900 && !(typeof ccRaceRivals==='function' && ccRaceRivals().length>=2);i++) await wait(100);
    if(ccRaceRivals().length<2) throw new Error('строки гонки соперников не пришли: '+ccRaceRivals().length);
    out.notes.rival=ccRaceRivals().map(p=>p.card.handle).join('+'); out.notes.rivalMates=ccRaceRivals().map(p=>(p.mates||[]).map(m=>m.handle).join('/')); out.notes.apart=ccRaceApartWhy(careerNext());
...
const C = {nick:process.env.CC_NICK_C||'LiveC', age:20, ageEdge:2, country:'fr', close:3, ovr:88, role:'roleFRG', role_mp:'b', money:5000, reach:100, form:1, grind:3, skipAt:SKIP_B, hideAt:0, reloadAt:0, mateIdx:2};
...
(async ()=>{
  ...
  const [a, b, c]=await Promise.all([runOne('A', A, P0), runOne('B', B, P0+1), runOne('C', C, P0+2)]);
  ...
  for(const [n, r] of [['A', a], ['B', b], ['C', c]]){ const t=(r.notes.table||[]).join('\n'); if(!/Your squad|Твой состав/.test(t)){ console.log('FAIL '+n+': в таблице нет своей строки'); bad++; }
    if(!r.notes.f1 || r.notes.f1.rivals!==2 || r.notes.f1.you<0){ console.log('FAIL '+n+': в поле нет обеих команд людей: '+JSON.stringify(r.notes.f1)); bad++; }
    console.log('   '+n+': напарник '+r.notes.mate+' · соперник '+r.notes.rival+' с '+JSON.stringify(r.notes.rivalMates)+' · врозь: '+r.notes.apart); }
  if(!DIFFER && !FF && !RELOAD_A && !RELOAD_B && (a.notes.rolls!==b.notes.rolls || a.notes.rolls!==c.notes.rolls)){ console.log('FAIL броски разные: '+a.notes.rolls+' / '+b.notes.rolls+' / '+c.notes.rolls); bad++; }
  if(bad) process.exit(1);
  console.log('гонка втроём: три живых клиента сыграли один вечер в одной комнате');
