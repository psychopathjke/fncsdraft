...
(async function(){
  ...
  try{
    ...
    localStorage.setItem('fncsdraft_career', JSON.stringify({
      ...
      career:{season:1, day:day, division:4, earnings:0, balance:1000, reach:0,
              tokens:[], log:[], news:[]},
      partner:null}));
    careerEntry();
    ...
    out.saveBytes=JSON.stringify(CAREER).length;
    const realNames=new Set(); PLAYERS.forEach(p=>{ if(p.handle) realNames.add(hKey(p)); });
    const bk=Object.keys((CAREER.dev)||{});
    out.ladderEntries=bk.filter(k=>!realNames.has(k)).length;
    out.realEntries=bk.filter(k=>realNames.has(k)).length;
    out.bookSize=Object.keys((CAREER.dev)||{}).length;
    out.summary={
...
console.log('сейв: '+out.saveBytes+' байт; в книге: реальных '+out.realEntries+', выдуманных '+out.ladderEntries);
