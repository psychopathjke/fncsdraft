...
(async function(){
  ...
  const seed = (day, div) => {
    ...
    /* Без пойча. После сильного вечера напарника переманивают с шансом 35%
       (careerMatePoach), и вторник без напарника недоступен — перемотка честно
       тренируется, а этот сторож требовал «все вечера недели сыграны» и падал
       через раз («1 rows for 2 cup nights», «a month plays more than a week»;
       найдено пробой 7 сентября 2026). Пойч — правило игры, у него свой сторож;
       здесь проверяется перемотка, поэтому уход напарника выключен. */
    careerMatePoach = () => null;
  };
  try {
